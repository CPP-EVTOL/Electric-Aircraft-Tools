...
%This script plots the constraint diagram for a fixed wing aircraft. It
%considers takeoff and landing, climb angle and rage, level turn, max
%airspeed, and service ceiling. Before using this script, read Gudmundsson
%section 3.2 about constraint diagrams. If additional performance
%parameters are needed (such as clumbing turn, etc) the equations can be
%found in this section as well.

%Many of the input parameters for this script are unknown during early
%conceptual design, so you must use either an "educated guess" or values
%from a similar aircraft, and then change them later. It's an iterative
%process.
